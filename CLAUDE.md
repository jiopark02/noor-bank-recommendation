...
⚠️ **`/api/plaid/disconnect` deletes the DB row without calling `itemRemove`,** so the Item stays live on Plaid's side. `/api/account/delete` does call it, best-effort, because it already loads the access token. Access tokens are stored **encrypted** in `plaid_connections` (PL1): AES-256-GCM at the application layer, written and read only by `src/lib/plaidTokenCrypto.ts`, in the form `v<n>:<iv>:<authTag>:<ciphertext>`. The version tag names a key VERSION, not a key VALUE; multi-version decrypt is not implemented, so one key is live at a time. The migration that introduces this also adds a `CHECK` constraint rejecting any value without the tag — whether that constraint is live is live state, so read the column's constraints before relying on it to keep plaintext out. The AAD is the owning `user_id`, taken verbatim from the verified token — a normalized or substituted id fails to decrypt, and that failure is invisible to `tsc` and to the unit tests. Every failure is fail-closed (no plaintext fallback in any configuration); the one exception is `/api/account/delete`, which logs and continues so a decrypt failure cannot block a user's deletion. Note the consequence once production credentials are in play: deleting a row discards the only copy of the token, so the Item can no longer be revoked at all — and losing `PLAID_TOKEN_ENCRYPTION_KEY` has the same effect on every row at once.
