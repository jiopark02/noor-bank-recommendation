...
- **Local Node exists, but it is not a build-verification path.** A Node toolchain is installed on the operator's machine (`node`, `npm`), repo at `C:\dev\noor`. Sanctioned uses are read-only or install-only checks: `tsc --noEmit`, `npm audit`, `npm ci`, `vitest run`. `npm run dev` / `npm run build` are NOT accepted verification — the machine holds no secrets (`.env.local` is prohibited there), so a local build cannot reproduce the deployed environment. Vercel remains the only build check; never claim a change is "verified" without a Vercel deploy. Whether `node_modules` matches `package-lock.json` is live state — run `npm ci` before trusting any local command.
- **Two automated checks run on push, not one.** Vercel builds (tsc type-check) and `.github/workflows/test.yml` runs `vitest run` on every push to main and every PR (#26+ runs exist — this CI predates our awareness of it). They cover different things: the Vercel build type-checks source (test files are excluded via tsconfig), while CI actually executes the tests. CI installs with `npm ci` on Node 24, so package.json/lockfile drift is a hard failure at the install step before any test runs. The test step is `npm run test --if-present`, which means a missing `test` script makes CI pass silently rather than fail. CI can also fail on GitHub infrastructure issues unrelated to our code (e.g. action download "Service Unavailable", runner acquisition errors) — this happened 3 times on 2026-08-07. When that's the failure mode, just Re-run all jobs; it isn't a code failure.
...
**Revocation and row deletion are paired, per connection.** `/api/plaid/disconnect` and `/api/account/delete` both call `itemRemove` first and delete the `plaid_connections` row **only** if that succeeded. Every failure — a Plaid error, a decrypt failure, a failed delete — leaves the row where it is. `src/lib/plaidRevocation.ts` owns that ordering and `src/lib/accountDeletion.ts` owns the account-deletion decision built on it; both take their dependencies as arguments, so the ordering is held by tests that execute it rather than by reading source. Two consequences to know before touching either route. First, account deletion **stops** while any connection is un-revoked: it returns 500 — or 503 with `PLAID_CRYPTO_UNCONFIGURED` when the failure was key-level, which is a configuration fault rather than something the user can retry — without touching `public.users` or the Auth user, so nothing outside `plaid_connections` is removed — but the connections processed before the failure are already gone, Item revoked and row deleted, because revocation runs per connection and is not rolled back. A partial run is therefore the normal shape of a failure, not an anomaly. Second, **no Plaid error code is treated as success**, including any that reads as "already removed" — whether one eventually should be is open and needs a live observation first, not a guess; the `[plaid-revoke] … failure=plaid error_code=` log line is what that observation will come from. Note the gate does not always clear on a retry: a row whose Item was revoked but whose delete failed, and a row with corrupt ciphertext under a working key, both fail identically forever, and such a user cannot complete an account deletion at all. There is no recovery path in the code today.

Access tokens are stored **encrypted** in `plaid_connections` (PL1): AES-256-GCM at the application layer, written and read only by `src/lib/plaidTokenCrypto.ts`, in the form `v<n>:<iv>:<authTag>:<ciphertext>`. The version tag names a key VERSION, not a key VALUE; multi-version decrypt is not implemented, so one key is live at a time. The migration that introduces this also adds a `CHECK` constraint rejecting any value without the tag — whether that constraint is live is live state, so read the column's constraints before relying on it to keep plaintext out. The AAD is the owning `user_id`, taken verbatim from the verified token — a normalized or substituted id fails to decrypt, and that failure is invisible to `tsc` and to the unit tests. **Every failure is fail-closed: no plaintext fallback in any configuration, and no exception anywhere.** There used to be one — `/api/account/delete` logged a decrypt failure and carried on, so a bad key could not block a deletion — and it was removed because its premise was wrong. A token that will not decrypt today is usually recoverable: the dominant cause is a reversible key configuration accident (a rotation that left the old key behind, Production and Preview carrying different values, a mistyped variable), and recovering the key makes the row readable and its Item revokable again. Deleting the row is what makes the loss permanent — the ciphertext is the only copy of the token, so a deleted row's Item can never be revoked by anyone. That fact is now the reason to KEEP a row whose Item was not revoked, rather than a cost accepted while deleting it. Losing `PLAID_TOKEN_ENCRYPTION_KEY` outright does the same thing to every row at once.
...
| `PLAID_TOKEN_ENCRYPTION_KEY` | Base64 of 32 random bytes; AES-256-GCM key for `plaid_connections.access_token`. Must carry the **same value on Production and Preview** — the two scopes share one Supabase project, so different keys would put mutually undecryptable ciphertext in one table. Sensitive, so it cannot be read back: keep a copy outside Vercel |
...
- **The local build is not a trusted check** → the Vercel deploy is the local-build-equivalent gate; CI (`npm run test --if-present` via `.github/workflows/test.yml`) covers tests separately, on every push. A local toolchain exists, but its sanctioned uses are `tsc --noEmit`, `npm audit`, `npm ci`, and `vitest run` — not `npm run build`. (Repeated because "the local build proves it" is the most common false assumption.)
